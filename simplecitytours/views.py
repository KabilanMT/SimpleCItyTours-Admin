...
from app.models import Polygon, LandingPage, Location, PointType, Point, Tourist, UserPayment, AdminActiveTime, Audio
...
def get_points(request):
    if request.method == 'GET':
        ...
        for i in range(len(all_locations)):
            ...
            points_in_location          = Point.objects.filter(location_id =all_locations[i].id)
            
            for j in range(len(points_in_location)):
                point = {}
                pointId             = points_in_location[j].id
                name                = points_in_location[j].name
                lat                 = points_in_location[j].lat
                lng                 = points_in_location[j].lng
...
class AdminActiveTimeViewSet(viewsets.ModelViewSet):
    queryset = AdminActiveTime.objects.all()
    serializer_class = AdminActiveTimeSerializer